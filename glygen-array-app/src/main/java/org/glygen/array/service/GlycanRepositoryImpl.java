...
public class GlycanRepositoryImpl extends GlygenArrayRepositoryImpl implements GlycanRepository  {
	...
	public String addGlycan(Glycan g, UserEntity user) throws SparqlException, SQLException {
		...
		switch (g.getType()) {
			case SEQUENCE_DEFINED:
				return addSequenceDefinedGlycan ((SequenceDefinedGlycan) g, user);
			case UNKNOWN:
	...
	private String addUnknownGlycan(Glycan g, UserEntity user) throws SparqlException, SQLException {
		String graph = null;
		if (g == null || g.getName() == null || g.getComment() == null)
			// cannot add 
			throw new SparqlException ("Not enough information is provided to register a glycan");
	...
	private String addSequenceDefinedGlycan(SequenceDefinedGlycan g, UserEntity user) throws SparqlException, SQLException {
		String graph = null;
		...
		if (existing == null) {
			...
			statements.add(f.createStatement(glycan, hasGlytoucanId, glytoucanId, graphIRI));
